...
export default function NavBar() {
    ...
    function handleCountry(country) {
       dispatch(setFilters({ country, order: selectedOrder, category: selectedCategory }))
    }

    function handleOrder(order) {
        dispatch(setFilters({ country: selectedCountry, order, category: selectedCategory }))
    }

    function handleCategory(category) {
        dispatch(setFilters({ country: selectedCountry, order: selectedOrder, category }))
    }
        ...
        <>
            <Navbar className={style.container}>
                <Container className={style.content}>
                    <NavDropdown title={ selectedCountry ?? "País de origen" } id="basic-nav-dropdown">
                        <NavDropdown.Item onClick={() => handleCountry(null)}>Todos</NavDropdown.Item>
                        {
                        ...
                        <NavDropdown.Item onClick={() => handleOrder(null)}>Defecto</NavDropdown.Item>
                        {
                            (configuration.order ?? []).map((item) => {
                               ...
                               onClick={() => handleOrder(item)}>
                                {item.name} { selectedOrder ? selectedOrder.id === item.id && "✔️" : <></> }
                                </NavDropdown.Item>
                            })
                        }
                    </NavDropdown>
